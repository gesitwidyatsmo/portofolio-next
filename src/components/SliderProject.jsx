    ...
    <div className="relative text-white lg:px-16 xl:px-32 xl:w-full xl:h-screen overflow-hidden object-cover">
      <Swiper
        ...
        {ProjectData.map((data, i) => (
          <SwiperSlide key={i}>
            <Image className="brightness-50 w-full " src={data.src} alt={data.title} width={900} height={200} />
      ...
      <div className="absolute bottom-2 sm:bottom-5 left-0 right-0 flex justify-center items-center gap-4 z-20">
        <button onClick={() => swiperRef.current?.slidePrev()} aria-label="Sebelumnya">
          <BsArrowLeftCircleFill size={35} className="dark:text-primary dark:border-white dark:border-2 dark:rounded-full dark:bg-white" />
        </button>
        <button onClick={() => swiperRef.current?.slideNext()} aria-label="Setelahnya">
          <BsArrowRightCircleFill size={35} className="dark:text-primary dark:border-white dark:border-2 dark:rounded-full dark:bg-white" />
        </button>
      </div>
      <div className="absolute w-32 sm:w-48 md:w-64 xl:w-96 bottom-2 sm:bottom-5 right-2 sm:right-5 lg:right-20 xl:right-36">
        <Swiper onSwiper={setThumbsSwiper} loop={true} spaceBetween={5} slidesPerView={2} freeMode={true} watchSlidesProgress={true} modules={[FreeMode, Navigation, Thumbs]} className="mySwiper">
          {ProjectData.map((data, i) => (
            <SwiperSlide key={i}>
              <Image className="rounded-lg object-cover overflow-hidden" src={data.imgH} alt={data.title} width={200} height={200} />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
